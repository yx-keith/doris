...
public class ExternalCatalogDeadlockTest {
    ...
    public void testResetToUninitializedShouldNotDeadlockWithCacheLoader() throws Exception {
        System.out.println("ExternalCatalogDeadlockTest is start on time " + new Date());
        ...
        LoadingCache<String, String> cache = Caffeine.newBuilder().build(key -> {
            System.out.println("start to load data key: " + key + " time:" + new Date());
            loaderEntered.countDown();
            awaitLatch(allowLoaderToTouchCatalog);
            System.out.println("catalog  before makeSureInitialized..");
            catalog.makeSureInitialized();
            System.out.println("catalog  after makeSureInitialized..");
            return key;
        });
        ...
        Thread refreshThread = new Thread(
                () -> runQuietly(backgroundFailure, () -> {
                    System.out.println("catalog refresh start on time " + new Date());
                    // resetToUninitialized() grabs the catalog monitor before invalidating the cache.
                    catalog.setInvalidator(() -> {
                        System.out.println("catalog refresh before countDown...");
                        allowLoaderToTouchCatalog.countDown();
                        System.out.println("catalog refresh after countDown...");
                        cache.invalidate("deadlock-key");
                    });
                    System.out.println("catalog resetToUninitialized is call  on " + new Date());
                    catalog.resetToUninitialized(true);
                    System.out.println("catalog resetToUninitialized call after on " + new Date());
                }),
    ...
    private static void assertNoDeadlock(Thread queryThread, Thread refreshThread,
        ...
        Assertions.assertNull(backgroundFailure.get(), new Date() + " unexpected background failure: "
    ...
    private static void awaitLatch(CountDownLatch latch) throws InterruptedException {
        System.out.println("latch is wait on time " + new Date());
        Assertions.assertTrue(latch.await(5, TimeUnit.SECONDS));
        System.out.println("latch is timeout " + new Date());
    }
    ...
    private static class DeadlockCatalog extends ExternalCatalog {
        ...
        public void  refreshOnlyCatalogCache(){

        }

        @Override
        public void onRefreshCache(boolean invalidCache) {
            System.out.println("catalog onRefreshCache is called..");
        ...
        @Override
        public java.util.List<String> listTableNames(SessionContext ctx, String dbName) {
