...
// buffering/building and release on finish. A budget of 0 (config default)
// disables admission control and all reservations succeed immediately.
//
...
public:
    ...
    // admission reservation. Because the build already holds bytes() it is
    // exempt from blocking against itself; it only waits for *other* concurrent
    // builds. Returns true on success (handle grows), false on timeout (handle
    // unchanged). `additional_bytes <= 0` is a no-op that returns true.
...
//
// `total_rows` is the number of rows that end up resident in the index
// (vector store + graph/lists). `buffered_rows` is the number of raw float
// rows held in the writer's input buffer at peak: the full segment for
// indexes that need training (the whole buffer must be alive for train()),
// or one streaming chunk for training-free indexes that add incrementally.
//
// Bytes accounted:
//   - input buffer (buffered_rows * dim * sizeof(float))
...
//   - temp workspace: max(input buffer, omp_threads * 4 MiB)
//
...
int64_t estimate_ann_build_memory(const AnnBuildMemoryParams& params, int64_t total_rows,
