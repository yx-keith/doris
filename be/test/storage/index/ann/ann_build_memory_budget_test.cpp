...
TEST_F(AnnBuildMemoryBudgetTest, EstimateGrowsWithRows) {
    AnnBuildMemoryParams p;
    p.index_type = AnnBuildMemoryParams::IndexKind::HNSW;
    p.quantizer = AnnBuildMemoryParams::Quantizer::FLAT;
    p.dim = 128;
    p.max_degree = 32;
    const int64_t small = estimate_ann_build_memory(p, /*total_rows=*/1000, /*buffered_rows=*/100);
    const int64_t big = estimate_ann_build_memory(p, /*total_rows=*/100000, /*buffered_rows=*/100);
    EXPECT_GT(big, small);
    EXPECT_GT(small, 0);
}

TEST_F(AnnBuildMemoryBudgetTest, EstimateBufferedRowsAddToFootprint) {
    // A trained-index build keeps the whole segment in the input buffer
    // (buffered_rows == total_rows); a streaming build only one chunk. The
    // estimate must charge for the difference once the buffer dominates the
    // fixed per-thread workspace floor.
    AnnBuildMemoryParams p;
    p.index_type = AnnBuildMemoryParams::IndexKind::HNSW;
    p.quantizer = AnnBuildMemoryParams::Quantizer::FLAT;
    p.dim = 512;
    ...
    const int64_t full_buffer = estimate_ann_build_memory(p, rows, /*buffered_rows=*/rows);
    const int64_t chunk_buffer = estimate_ann_build_memory(p, rows, /*buffered_rows=*/1000);
    EXPECT_GT(full_buffer, chunk_buffer);
}
...
protected:
    void SetUp() override {
        ...
        _saved_timeout = config::ann_index_build_memory_wait_timeout_ms;
        _saved_action = config::ann_index_build_on_oom_action;
        _saved_chunk_bytes = config::ann_index_build_chunk_bytes;
        AnnBuildMemoryBudget::instance().reset_for_test();
    ...
    void TearDown() override {
        ...
        config::ann_index_build_memory_wait_timeout_ms = _saved_timeout;
        config::ann_index_build_on_oom_action = _saved_action;
        config::ann_index_build_chunk_bytes = _saved_chunk_bytes;
    }

    int64_t _saved_budget = 0;
    int64_t _saved_timeout = 0;
    std::string _saved_action;
    int64_t _saved_chunk_bytes = 0;
...
TEST_F(WriterAdmissionTest, ReservationGrowsWithAccumulatedRows) {
    // With a generous budget the writer must enlarge its reservation as rows
    // accumulate, so the global counter tracks real memory rather than just
    // the initial chunk-sized floor.
    ...
    // Shrink the chunk so the initial floor covers a single row and the adds
    // below force the reservation to grow (dim=16 -> 64 bytes per row).
